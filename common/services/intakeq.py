...
from common.models.intakeq.questionnaires import (
    Intake,
    Questionnaire,
    ResendIntakeRequest,
)
...
class IntakeQService(BaseService):
    ...
    def get_clients(
        ...
        **kwargs,
        ...
        params = {
            ...
            "dateCreatedStart": date_created_start.strftime("%Y-%m-%d")
            if date_created_start
            else None,
            "dateCreatedEnd": date_created_end.strftime("%Y-%m-%d")
            if date_created_end
            else None,
            "dateUpdatedStart": date_updated_start.strftime("%Y-%m-%d")
            if date_updated_start
            else None,
            "dateUpdatedEnd": date_updated_end.strftime("%Y-%m-%d")
            if date_updated_end
            else None,
        ...
        response = self.api_call(method="get", endpoint="clients", params=params)
    ...
    def get_client_diagnoses(self, client_id: int) -> List[Diagnosis]:
        response = self.api_call(method="get", endpoint=f"client/{client_id}/diagnoses")
    ...
    def get_appointments(
        ...
        params = {
            ...
            "page": page,
            "updatedSince": updated_since.strftime("%Y-%m-%d")
            if updated_since
            else None,
            "deletedOnly": deleted_only,
        }
        params = {k: v for k, v in params.items() if v is not None}
        response = self.api_call(method="get", endpoint="appointments", params=params)
    ...
    def get_appointment(self, appointment_id: str) -> Appointment:
        response = self.api_call(
            method="get", endpoint=f"appointments/{appointment_id}"
        )
        return Appointment.model_validate(response)

    def get_appointment_settings(self) -> AppointmentSettings:
        response = self.api_call(method="get", endpoint="appointments/settings")
        return AppointmentSettings.model_validate(response)

    def create_appointment(self, appointment: CreateAppointmentRequest) -> Appointment:
        response = self.api_call(
            method="post",
    ...
    def update_appointment(self, appointment: UpdateAppointmentRequest) -> Appointment:
        response = self.api_call(
            method="put",
            endpoint="appointments",
            json=appointment.model_dump(
    ...
    def cancel_appointment(self, appointment: CancelAppointmentRequest) -> Appointment:
        response = self.api_call(
            method="post",
            endpoint="appointments/cancellation",
            json=appointment.model_dump(
                by_alias=True, exclude_none=True, exclude_unset=True
            ),
        )
    ...
    def get_intake_forms(
        ...
        params = {k: v for k, v in params.items() if v is not None}
        response = self.api_call(
            method="get", endpoint="intakes/summary", params=params
        )
        return [Intake.model_validate(intake) for intake in response]

    def get_intake(self, intake_id: str) -> Intake:
        response = self.api_call(method="get", endpoint=f"intakes/{intake_id}")
        return Intake.model_validate(response)

    def get_questionnaires(self) -> List[Questionnaire]:
        response = self.api_call(method="get", endpoint="questionnaires")
        return [
            Questionnaire.model_validate(questionnaire) for questionnaire in response
        ]

    def get_practitioners(self) -> List[Practitioner]:
        response = self.api_call(method="get", endpoint="practitioners")
        return [Practitioner.model_validate(practitioner) for practitioner in response]

    def send_questionnaire(self, questionnaire: Questionnaire) -> Intake:
        response = self.api_call(
            method="post",
            endpoint="intakes/send",
    ...
    def resend_questionnaire(
        self, resend_intake_request: ResendIntakeRequest
    ) -> Intake:
        response = self.api_call(
            method="post",
            endpoint="intakes/resend",
            json=resend_intake_request.model_dump(
                by_alias=True, exclude_none=True, exclude_unset=True
            ),
        )
        return Intake.model_validate(response)
