...
from common.models.northtext.contacts import (
    ContactsResponse,
...
from common.models.northtext.webhooks import (
...
class NorthTextService(BaseService):
    ...
    def api_call(
        self, method: str, endpoint: str, data: str = None, json: dict | list = None
    ) -> dict:
    ...
    def get_self(self) -> AccountResponse:
        response = self.api_call(method="get", endpoint="/api/v2/account")
        return AccountResponse.model_validate(response)

    def get_users(self) -> UsersResponse:
        response = self.api_call(method="get", endpoint="/api/v2/user")
        return UsersResponse.model_validate(response)

    def get_message(self, message_id: int) -> MessageResponse:
        response = self.api_call(method="get", endpoint="/api/v2/message/{message_id}")
    ...
    def get_all_messages_by_tag(self, tag_name: str, tag_value: str) -> List[Message]:
        page = 0
        ...
        while len(messages_response.result) == 100:
            ...
            messages_response = self.get_messages(
                page=page, tag_name=tag_name, tag_value=tag_value
            )
            messages += messages_response.result
    ...
    def send_messages(self, messages: List[MessageSendRequest]) -> BulkMessagesResponse:
        response = self.api_call(
            method="post",
            endpoint="/api/v2/message/bulk",
            json=[
                m.model_dump(by_alias=True, exclude_unset=True, exclude_none=True)
                for m in messages
            ],
        )
    ...
    def get_contact(self, contact_id: int) -> ContactResponse:
        response = self.api_call(method="get", endpoint=f"/api/v2/contact/{contact_id}")
        return ContactResponse.model_validate(response)
    ...
    def delete_webhook(self, webhook_id: int) -> WebhookDeleteResponse:
        response = self.api_call(
            method="delete", endpoint=f"/api/v2/webhook/{webhook_id}"
        )
