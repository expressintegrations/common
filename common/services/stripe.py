...
class StripeService(BaseService):
    ...
    def get_billing_portal_configuration(
        self, base_url, integration: Integration, products: List[Product]
    ):
        ...
        config = self.stripe_client.billing_portal.Configuration.create(
            ...
            metadata={"integration_id": integration.name},
        )
    ...
    def create_billing_session(self, customer_id: str, billing_portal_config_id: str):
        try:
            return self.stripe_client.billing_portal.Session.create(
                customer=customer_id, configuration=billing_portal_config_id
            )
        except Exception as e:
            print(e)
            return {"error": {"message": str(e)}}

    def create_billing_portal_configuration(
        self, base_url: str, features, integration_label: str, integration_id: str
    ):
    ...
    def get_billing_session(
        ...
        try:
            billing_portal_config_id = self.get_billing_portal_configuration(
                base_url=base_url, integration=integration, products=products
            )
    ...
    def cancel_subscription(self, subscription_id: str):
        self.update_subscription(
            subscription_id=subscription_id, cancel_at_period_end=True
        )

    def update_subscription(self, subscription_id: str, **kwargs):
        self.stripe_client.Subscription.modify(subscription_id, **kwargs)
    ...
    def create_usage_record(self, subscription_item_id: str, quantity):
        return self.stripe_client.SubscriptionItem.create_usage_record(
            subscription_item_id, quantity=quantity, action="set"
        )
