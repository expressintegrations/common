...
class SnowflakeService(BaseService):
    ...
    def replicate_table_schema(
        ...
        if table not in [row[1] for row in rows]:
            snowflake_column_definitions = ', '.join(f"{c['name']} {c['type']}" for c in column_definitions)
            self.execute(
            ...
            columns_to_add = [f"{c['name'].lower()} {c['type']}" for c in column_definitions if
                              c['name'].lower() not in current_column_names]
            ...
            columns_to_drop = [
                c[2].lower() for c in current_columns
            ...
            columns_to_drop_retype = [
                c[2].lower() for c in current_columns
                if json.loads(c[3])['type'] != column_definitions_map[c[2].lower()]['type']
            ]
            ...
            columns_to_add_retype = [
                f"{c['name']} {c['type']}" for c in column_definitions if c['name'].lower() in columns_to_drop_retype
            ]
