...
class AnvilService(BaseService):
    ...
    def get_account_by_stripe_customer_id(self, customer_id: str, keep_alive: bool = True):
        if not self.connected:
            self.connect()
        account = app_tables.accounts.get(
    ...
    def get_subscription_by_stripe_id(self, subscription_id: str, keep_alive: bool = True):
        if not self.connected:
            self.connect()
